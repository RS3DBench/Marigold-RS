...
class BaseDepthDataset(Dataset):
    def __init__(
        ...
        self.text_prompt_path = kwargs.get("text_prompt_path", None)
    ...
    def __getitem__(self, index):
        ...
        # if torch.isnan(outputs['depth_raw_norm']).any():
        #     print("NaN detected")
        return outputs

    def _load_text_data(self, rgb_rel_path):
        if self.text_prompt_path is None:
            return {}
        # name = os.path.join(self.text_prompt_path, os.path.basename(rgb_rel_path)[:-4] + ".txt")
        with open(os.path.join(self.text_prompt_path, os.path.basename(rgb_rel_path)[:-4] + ".txt"), "r") as f:
            text = f.read()
        # print(f"{name}: {text}")
    ...
    def _get_data_item(self, index):
        ...
        rasters.update(self._load_text_data(rgb_rel_path))
        ...
        if DatasetMode.RGB_ONLY != self.mode:
            ...
            # ).clone()
            # if not rasters["valid_mask_raw"].all() or not rasters["valid_mask_filled"].all():
            #     raise RuntimeError("有数据不在0~1内！")
            # for k, v in rasters.items():
            #     # 检查nan
            #     if torch.isnan(v).any():
            #         raise RuntimeError(f"{k}数据为nan！")
    ...
    def _load_rgb_data(self, rgb_rel_path):
        ...
        rgb_norm = self._read_rgb_file(rgb_rel_path) / 255.0 * 2.0 - 1.0  # [0, 255] -> [-1, 1]
    ...
    def _training_preprocess(self, rasters):
        ...
        rasters["depth_raw_norm"] = self.depth_transform(
            rasters["depth_raw_linear"], None if "valid_mask_raw" not in rasters else rasters["valid_mask_raw"]
        ).clone()
    ...
    def _augment_data(self, rasters_dict):
        ...
        if random.random() < lr_flip_p:
            rasters_dict = {k: v.flip(-1) if type(v) != str else v for k, v in rasters_dict.items()}
