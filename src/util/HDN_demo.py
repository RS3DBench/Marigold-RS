...
def masked_shift_and_scale(depth_preds, depth_gt, mask_valid):
    if mask_valid is None:
        mask_valid = torch.ones_like(depth_preds).bool()
...
def get_contexts_dr(level, depth_gt, mask_valid):
    if mask_valid is None:
        mask_valid = torch.ones_like(depth_gt).bool()
    batch_norm_context = []
...
def get_contexts_ds(level, mask_valid):
    if mask_valid is None:
        mask_valid = torch.ones_like(depth_gt).bool()
    templete_contexts = init_temp_masks_ds(level, mask_valid.shape[-1]).to(mask_valid.device)
...
class SSIMAE(nn.Module):
    ...
    def forward(self, depth_preds, depth_gt, mask_valid, dense):
        depth_pred_aligned, depth_gt_aligned = masked_shift_and_scale(depth_preds, depth_gt,
                                                                      mask_valid)  # normalize the depth maps
...
def compute_hdn_loss(SSI_LOSS, depth_preds, depth_gt, mask_valid_list):
    ...
    hdn_loss_level_list = hdn_loss_level_list.sum(
        dim=0)  # summed loss generated by  different contexts  for all locations
    ...
    hdn_loss = hdn_loss_level_list.sum() / torch.ones_like(
        hdn_loss_level_list).sum()  # average the losses of all locations
...
class HDN_interface:
    ...
    def __init__(self, **kwargs):
        self.alpha = kwargs['alpha'] if 'alpha' in kwargs else 0.33
        self.beta = kwargs['beta'] if 'beta' in kwargs else 0.33
        self.gamma = kwargs['gamma'] if 'gamma' in kwargs else 0.33

    def __call__(self, depth_preds, depth_gt, mask_valid=None, **kwargs):
        if mask_valid is None:
            mask_valid = torch.ones_like(depth_preds).bool()
        SSI_LOSS = SSIMAE()
        mask_valid_list_dr = get_contexts_dr(3, depth_gt, mask_valid)
        mask_valid_list_dp = get_contexts_dp(3, depth_gt, mask_valid)
        mask_valid_list_ds = get_contexts_ds(3, mask_valid)
        ...
        return self.alpha * loss_hdn_dr + self.beta * loss_hdn_dp + self.gamma * loss_hdn_ds
...
if __name__ == '__main__':
    ...
    depth_preds = torch.rand(2, 1, 384, 384).to(device)  # predicted depth maps
    depth_gt = torch.rand(2, 1, 384, 384).to(device)  # ground truth depth maps
    mask_valid = (torch.rand(2, 1, 384, 384) > -1).to(device)  # valid pixels
