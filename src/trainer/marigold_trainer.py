...
class MarigoldTrainer:
    def __init__(
            ...
            # model: MarigoldReferencePipeline,
            ...
            text_prompt_path=None
    ):
        self.cfg: OmegaConf = cfg
        self.model: MarigoldPipeline = model
        # self.model: MarigoldReferencePipeline = model
        self.device = device
        ...
        if hasattr(model, "unet"):
            ...
            if 8 != self.model.reference_unet.config["in_channels"]:
                self._replace_unet_conv_in()
            if 8 != self.model.marigold_unet.config["in_channels"]:
                self._replace_unet_conv_in()

        # Encode empty text prompt
        if self.text_prompt_path is not None:
            if len(self.text_prompt_path) != 1:
                # raise ValueError("Only one text prompt is supported.")
                print("加载混合的text_prompt_path")
            else:
                print(f"加载文本提示：{self.text_prompt_path[0]}")
                self.text_prompt_path = self.text_prompt_path[0]
            # self.model.encode_text_prompt = self.model.encode_text(self.text_prompt_path).to(device)
            self.model.empty_text_embed = None
            self.empty_text_embed = None
        else:
            self.model.encode_text_prompt = None
        ...
        if self.text_prompt_path is None:
            self.model.text_encoder.requires_grad_(False)
        else:
            self.model.text_encoder.requires_grad_(True)
        # self.model.unet.requires_grad_(True)
        if hasattr(self.model, "unet"):
            self.model.unet.requires_grad_(True)
        else:
            self.model.marigold_unet.requires_grad_(True)
        ...
        if hasattr(self.model, "unet"):
            ...
            self.optimizer = Adam(list(self.model.marigold_unet.parameters()), lr=lr
                                  )

        if self.text_prompt_path is not None:
            self.optimizer.add_param_group(
                {"params": self.model.text_encoder.parameters(), "lr": lr}
            )
        ...
        self.gt_mask_type = None if "gt_mask_type" not in self.cfg else self.cfg.gt_mask_type
    ...
    def train(self, t_end=None):
        ...
        for epoch in range(self.epoch, self.max_epoch + 1):
            ...
            for batch in skip_first_batches(self.train_loader, self.n_batch_in_epoch):
                # if self.effective_iter < 4:
                #     print(f"skip {self.effective_iter}")
                #     self.effective_iter += 1
                #     # if self.effective_iter == 3:
                #     #     print("Debug")
                #     continue
                # if torch.isnan(batch[self.gt_depth_type]).any():
                #     print(f"in {self.effective_iter}'s {self.gt_mask_type} contains NaN.")
                #     self.effective_iter += 1
                #     continue
                ...
                # Text embedding
                if self.text_prompt_path is not None:  # 从batch["text"]中获取文本
                    text_embed = self.model.encode_text(batch["text"]).to(device)
                else:
            ...
            # val_loader = self.val_loaders[0]  # 使用第一个验证集
            # val_loss = 0
            # n_val_batches = 0
            #
            # with torch.no_grad():
            ...
            #         # Get validation data
            #         rgb = val_batch["rgb_norm"].to(device)
            #         depth_gt_for_latent = val_batch['depth_raw_linear'].to(device)
            #
            #         if self.gt_mask_type is not None:
            #             valid_mask_for_latent = val_batch[self.gt_mask_type].to(device)
            #             invalid_mask = ~valid_mask_for_latent
            #             valid_mask_down = ~torch.max_pool2d(
            #                 invalid_mask.float(), 8, 8
            #             ).bool()
            #             valid_mask_down = valid_mask_down.repeat((1, 4, 1, 1))
            #
            #         # Encode image and depth
            #         rgb_latent = self.model.encode_rgb(rgb)
            #         gt_depth_latent = self.encode_depth(depth_gt_for_latent)
            #
            #         # Use fixed timestep for validation (e.g., middle of sequence)
            #         batch_size = rgb.shape[0]
            #         timesteps = torch.ones((batch_size,), device=device).long() * (self.scheduler_timesteps // 2)
            #
            #         # Add noise
            #         noise = torch.randn(gt_depth_latent.shape, device=device)
            #         noisy_latents = self.training_noise_scheduler.add_noise(
            #             gt_depth_latent, noise, timesteps
            #         )
            #
            ...
            #
            #         # Concat and predict
            #         cat_latents = torch.cat([rgb_latent, noisy_latents], dim=1)
            ...
            #             )
            #         else:
            #             batch_loss = self.loss(model_pred.float(), target.float())
            #
            ...
            #
            # logging.info(
            #     f"Validation loss at epoch {epoch}: {avg_val_loss:.5f}，now time is {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))}")
            ...
            #     )
            #     logging.info(f"New best model saved with validation loss: {avg_val_loss:.5f}")

            # Continue with original end-of-epoch logic
            if self.max_iter > 0 and self.effective_iter >= self.max_iter:
                self.save_checkpoint(
                    ckpt_name=self._get_backup_ckpt_name(),
                    save_train_state=False,
    ...
    def save_checkpoint(self, ckpt_name, save_train_state):
        ...
        if self.text_prompt_path is not None:
            ...
            self.model.text_encoder.save_pretrained(text_encoder_path)
            logging.info(f"Text encoder is saved to: {text_encoder_path}")
    ...
    def load_checkpoint(
        ...
        if type(self.device) == int:
            self.model.unet.load_state_dict(
                torch.load(_model_path, map_location=f"cuda:{self.device}")
            )
        else:
            self.model.unet.load_state_dict(
        ...
        # 如果text_encoder存在，则加载
        if self.text_prompt_path is not None:
            _text_encoder_path = os.path.join(ckpt_path, "text_encoder", "model.safetensors")
            self.model.text_encoder.load_state_dict(
                load_file(_text_encoder_path, device=self.device)
            )
            self.model.text_encoder.to(self.device)
            logging.info(f"加载... Text encoder parameters are loaded from {_text_encoder_path}")
        torch.cuda.empty_cache()
        torch.clear_autocast_cache()
        # Load training states
        if load_trainer_state:
            checkpoint = torch.load(os.path.join(ckpt_path, "trainer.ckpt"), map_location="cpu")
