...
# from marigold.marigold_reference_pipline import MarigoldReferencePipeline
...
os.environ["WANDB_API_KEY"] = "00d6da9c8fac32c1af1a0115e8f23e4c1b1a7b42"
os.environ["WANDB_MODE"] = "offline"
os.environ["http_proxy"] = "http://127.0.0.1:7890"
os.environ["https_proxy"] = "http://127.0.0.1:7890"


def create_tarfile(_code_snapshot_path, _temp_code_dir):
    with tarfile.open(_code_snapshot_path, "w") as tar:
        tar.add(_temp_code_dir, arcname=os.path.basename(_temp_code_dir))


def remove_directory(_temp_code_dir):
    shutil.rmtree(_temp_code_dir)


def copy_files_with_exclusions(src_dir, dest_dir, exclude_dirs=None, gitignore_file=None):
    ...
    gitignore_patterns = []
    if gitignore_file and Path(gitignore_file).exists():
    ...
    for root, dirs, files in os.walk(src_dir):
        ...
        if dest_subdir.startswith(dest_dir):
            continue
        ...
        for file in files:
            ...
            if any(Path(file_path).match(pattern) for pattern in gitignore_patterns):
                continue
...
def main():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="config/train_marigold_JK.yaml",
    ...
    if resume_run is not None:
        ...
        if output_dir is not None:
            ...
            out_dir_run = os.path.join("/nfs5/wrz/src/python/Marigold/output", job_name)
    ...
    if resume_run is None:
        ...
        copy_files_with_exclusions(src_dir, _temp_code_dir, exclude_dirs,
                                   gitignore_file)
        # os.system(f"tar -cf {_code_snapshot_path} {_temp_code_dir}")
        # os.system(f"rm -rf {_temp_code_dir}")
        create_tarfile(_code_snapshot_path, _temp_code_dir)
        remove_directory(_temp_code_dir)
    ...
    train_dataset: BaseDepthDataset = get_dataset(
        ...
        text_prompt_path=cfg.text_prompt_path if hasattr(cfg, "text_prompt_path") else None
    )
    ...
    # 设置代理
    # _pipeline_kwargs["proxies"] = {'http': 'http:127.0.0.1:7890', 'https': 'https:127.0.0.1:7890'}
    model = MarigoldPipeline.from_pretrained(base_ckpt_dir + "/" + cfg.model.pretrained_path.replace("\\", "/"),
                                             **_pipeline_kwargs)
    ...
    # 打印出训练集和验证集的长度
    if "mixed" == cfg_data.train.name:
        logging.info(f"Training dataset length: {[len(ds) for ds in dataset_ls]}")
    else:
        logging.info(f"Train dataset length: {len(train_dataset)}")
    # logging.info(f"Validation dataset length: {[len(val_loader.dataset) for val_loader in val_loaders]}")
