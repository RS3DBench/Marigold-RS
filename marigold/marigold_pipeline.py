...
class MarigoldPipeline(DiffusionPipeline):
    ...
    def encode_text(self, text):
        ...
        """
        text_inputs = self.tokenizer(  # 这里的tokenizer是CLIPTokenizer
            text,
            padding=True,
            max_length=self.tokenizer.model_max_length,
            truncation=True,
        ...
        text_input_ids = text_inputs.input_ids.to(self.text_encoder.device)
        text_embed = self.text_encoder(text_input_ids)[0].to(self.dtype)  # 这里的text_encoder是CLIPTextModel
        return text_embed
    ...
    def single_infer(
        ...
        if text_prompt is not None:
            ...
            print(f"load text: {text_prompt}")
            batch_empty_text_embed = text_embed.repeat(
        ...
        for i, t in iterable:
            ...
            if img_name is not None and save_intermediate_path is not None:
                # 将depth_latent解码并保存
                depth_img = self.decode_depth(depth_latent)
                # 做个平均
                depth_img = torch.mean(depth_img, dim=0, keepdim=True).squeeze()
                depth_img = (depth_img - depth_img.min()) / (depth_img.max() - depth_img.min()) * 255
                depth_img = depth_img.squeeze().cpu().numpy().astype(np.uint8)
                depth_img = Image.fromarray(depth_img)
                depth_img.save(os.path.join(save_intermediate_path, f"{img_name}_depth_step_{i}.png"))
    ...
    def decode_rgb(self, rgb_latent: torch.Tensor) -> torch.Tensor:
        """
        Decode rgb latent into rgb map.
